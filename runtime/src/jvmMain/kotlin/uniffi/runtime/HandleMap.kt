...
//
// Since uniffi 0.30 both Rust-created and foreign-created handles travel across the FFI
// for trait interfaces, and the two are told apart by the lowest bit: it is always set
// for foreign handles and never for Rust ones. Starting at 1 and stepping by 2 keeps
// every handle we hand out odd. (Starting at 1 also avoids "Null Pointers" in native's
// `interpretCPointer`.)
...
class UniffiHandleMap<T: Any> {
    ...
    // Added in uniffi 0.30, which put `uniffi_clone` in the callback interface vtable.
