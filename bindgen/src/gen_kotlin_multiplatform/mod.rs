...
impl KotlinCodeOracle {
    ...
    fn ffi_type_label_by_value_header(&self, ffi_type: &FfiType) -> String {
        match ffi_type {
            FfiType::RustBuffer(_) => format!("{}", self.ffi_type_label_header(ffi_type)),
            FfiType::Struct(name) => format!("{}", self.ffi_struct_name_header(name)),
            _ => self.ffi_type_label_header(ffi_type),
        }
    }
    ...
    fn ffi_type_label_for_ffi_struct_header(&self, ffi_type: &FfiType) -> String {
        match ffi_type {
            // Make callbacks function pointers nullable. This matches the semantics of a C
            // function pointer better and allows for `null` as a default value.
            FfiType::Callback(name) => format!("{}?", self.ffi_callback_name_header(name)),
            _ => self.ffi_type_label_by_value_header(ffi_type),
        }
    }
...
mod filters {
    ...
    pub fn ffi_type_name_for_ffi_struct_header(type_: &FfiType) -> Result<String, askama::Error> {
        Ok(KotlinCodeOracle.ffi_type_label_for_ffi_struct_header(type_))
    }
