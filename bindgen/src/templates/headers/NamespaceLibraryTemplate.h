...
{%- match def %}
{%- when FfiDefinition::CallbackFunction(_) %}
{%- when FfiDefinition::Struct(ffi_struct) %}
typedef struct {{ ffi_struct.name()|ffi_struct_name }} {
    {% for field in ffi_struct.fields() %}
    {{ field.type_().borrow()|ffi_type_name_for_ffi_struct_header }} {{ field.name()|var_name_raw }};
    {% endfor %}
...
{% match func.return_type() -%}{%- when Some with (type_) %}{{ type_|header_ffi_type_name }}{% when None %}void{% endmatch %} {{ func.name() }}(
    {%- if func.arguments().len() > 0 %}
        {%- for arg in func.arguments() %}
            {{- arg.type_().borrow()|header_ffi_type_name }} {{ arg.name()|unquote -}}{% if !loop.last || func.has_rust_call_status_arg() %}, {% endif %}
        {%- endfor %}
